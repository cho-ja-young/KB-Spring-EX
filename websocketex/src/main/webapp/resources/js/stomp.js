...
//         // 이전 방 구독 취소 (unsubscribe는 StompJs 7에서 직접 저장 안하면 불가하므로 단순 구독만 허용)
//         // 실무에서는 client.unsubscribe(id)도 함께 관리
//     }
//
...
//         showMessage(`[${roomId}] ${msg.name}: ${msg.content}`);
//     });
// }
//
// // 연결됬을 때 엘리먼트 프로퍼티 변경
...
//             data.forEach(room => {
//                 const li = document.createElement('li');
//                 li.innerText = `${room.name} (${room.headCount})`;
//                 li.style.cursor = 'pointer';
//                 li.onclick = () => connectToWebSocket(room.id);
//                 ul.appendChild(li);
//             });
//         });
// }
